...
for input_text in input_texts:
    ...
    # 对剩下的单个词进行mask并预测
    if len(tokens) % 2 != 0:
        single_masked_tokens = tokens.copy()
        single_masked_tokens[-1] = "[MASK]"
        single_masked_text = " ".join(single_masked_tokens)

        single_inputs = tokenizer(single_masked_text, return_tensors="pt", padding=True, truncation=True)
        with torch.no_grad():
            single_outputs = model(**single_inputs)
            single_predictions = single_outputs.logits

        single_predicted_index = torch.argmax(single_predictions[0, len(single_masked_tokens) - 1]).item()
        single_predicted_token = tokenizer.convert_ids_to_tokens([single_predicted_index])[0]

        print(f"预测遮蔽 '{tokens[-1]}' 后的结果为: {single_predicted_token}")
